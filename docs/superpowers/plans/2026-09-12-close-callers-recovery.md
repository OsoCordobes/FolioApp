...
- Added prerequisite after real reproduction: complete and independently approve `2026-09-12-payment-settlement-authority.md` (M121) before implementing the shared settlement helper. Its reviewed RPC replaces the old REST UPDATE; preserving the old update algorithm is no longer sufficient or permitted for this path. Retain the existing wrapper capability gates, same-payment idempotency and honest confirmation semantics.
...
- [ ] Preserve the Finanzas wrapper's canSeeFinanzas plus canRegistrarCobro gate. Put its shared settlement behavior behind the reviewed M121 RPC; do not retain a direct REST UPDATE fallback. Add an agenda settlement wrapper gated canRegistrarCobro and binding the payment to the requested closed turno, active organization and current professional scope, with the same requirements enforced in DB. ASISTENTE can complete collection there without gaining access to Finanzas or clinical records. Return current persisted payment, including identity and updatedAt; preserve uncertainty after lost response and never invent another registration/receipt mechanism for settlement.
...
- [ ] Apply the same post-commit cache rule to `saveSesionFichaAction` in this owned file: its SAVE/AUTOSAVE receipt must survive revalidation failure too. Add an actual-action regression for both save and close, without changing their revision/idempotency contracts.
...
- [ ] Treat receipt `puedeRegistrar` as an immutable historical field, not permission for a current action. M120 may replay a nonfinancial receipt after the member changes role, preserving the old flag and redaction. Gate controls with current capabilities and freshly authorized status; never restore payment controls from that historical receipt. Tests must cover both promotion and demotion around a lost-response probe.
...
- [ ] Update the local runtime to the approved branch snapshot, inspect its known synthetic state and apply only the missing reviewed M120 and M121 migrations with canonical ledger entries. Activate M120 then M121 via their audited local administration functions after compatible code is present, preserving the already active M106 prerequisite. Do not reset the existing local fixtures or read production environment files.
...
Timestamp limit to resolve before Task2: M09 uses the transaction's `now()` for updated_at. It is useful freshness evidence but is not a strict commit-order revision across arbitrary concurrent transactions. Do not market or test it as one. Within the supported close/settlement path, use operation outcome and authorized current-state reads for an ambiguous ordering instead of guessing. No additional schema revision mechanism is authorized by this plan without a demonstrated need and root ruling.

Root ruling after the three actual settlement races: the initial helper-extraction recommendation preserved an unsafe database write under lock waits. Reassignment, revocation and reception scope loss all left a waiting UPDATE able to settle, despite fresh statements correctly denying access. M121 is now a required database prerequisite. The existing zero-row correction remains valid for confirmation behavior, but its tests are not proof of race-safe authorization; adapt them to the real new adapter boundary and retain equivalent negative/success assertions.
