...
**Literal-selector follow-up:** Run 8 passed the corrected reload check, then waited for exact `Subir`, while the actual component renders `Subir documento`. Correct that exact label and audit remaining selectors against the actual rendered component text before another run. A literal mismatch does not require another observer fixture to establish it. These corrections may only align selectors with existing controls; preserve actions, values, permissions, negative assertions and limits. Any actual product failure remains a separately reported RED; do not modify the baseline product to force this suite green.
