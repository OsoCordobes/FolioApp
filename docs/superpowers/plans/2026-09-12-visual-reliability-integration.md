...
**Post-merge reproduced defect:** Checkpoint `2696edc` passes types/lint/2126 units/build/reschedule/archive. Creation smoke initially passed 19/20; a bounded independent investigation then captured a real timer race: focus moved from Teléfono to Nombre between focus and input, appending the phone to the name and leaving creation disabled before any request. The modal source was unchanged by the merge. Add a deterministic browser case controlling only the pending 50 ms focus callback; reproduce focus stealing, then guard the callback so it moves focus only from body/the dialog container and never steals an already selected control. Keep initial focus, mode controls, focus trap, dismissal, frozen creation and retry behavior. Verify existing creation cases, types/lint/unit checks after the scoped TS edit; independently review the fix along with the merge. Do not increase test timeouts or add sleeps to hide this race.
