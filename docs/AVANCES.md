...
**Equipo comprobado:** A dirige e integra. `patient_form_delivery` corrige el formulario según la revisión de `intake_cancel_review`; `intake_sql_repair` diseña la recuperación de enlaces y `access_proof_repair` prepara el ensayo completo. Máximo dos escritores, copias y archivos separados. El refuerzo de permisos ya está publicado.
...
| **Ficha completada por el paciente** · base integrada; formulario en revisión | [PR186](https://github.com/OsoCordobes/FolioApp/pull/186) integrada tras aprobar sus pruebas; instalación confirmada sin alterar los datos previos. El formulario móvil y QR ya existen en la copia de prueba; la revisión encontró tres casos de conexión interrumpida que se corrigen antes de publicarlo. |
...
**Última publicación comprobada:** [PR185 · refuerzo de permisos](https://github.com/OsoCordobes/FolioApp/pull/185), también aprobó su control posterior. La base de la ficha está integrada y se verifica ahora su publicación. Para probar mejoras visibles anteriores: Configuración → Pantallas de espera, luego Hoy → Llamar; y Mis datos y solicitudes → Archivo clínico para guardar historia y archivos. **Cuota consultada el 26/09:** 96% disponible; no es una lectura en tiempo real. Reserva de cierre: 15%.
