...
- Dos permisos de escritura de implementación: **A (integración) y D (B03) actualmente**. B cerró B02a y pasa a revisión; C recibió una ventana operativa local acotada, sin cambios de aplicación. La edición administrativa del tablero pertenece a A.
...
| B02b | Alta / reforma solicitada por titular | B / D + A | diseño del contrato, implementación tras M125 | Primera elección antes de registro: Profesional independiente o Clínica, sin valor implícito. Dos recorridos pertinentes, precios canónicos por modalidad, responsable clínico/administrativo diferenciado, retomar tras email/OAuth/reload con DB autoritativa. No dar por publicado/listo un perfil o agenda incompletos. Calidad visual y pruebas reales de ambos recorridos antes del cierre |
...
| B03 | Alta / brecha de implementación identificada | D / B | implementación autorizada, separada de PR166 | Calendario consulta turno_extendido, incompatible con RLS de recepción, según código y prueba SQL M122 existente; síntoma live no reproducido. M124 compondrá M122 para rango <=42 fechas, semana/mes por rol, sin datos clínicos/financieros; SQL/UI/tenant negativos pendientes |
...
| B07 | Alta / defecto comprobado en código y regresión pendiente | B / D | M125 preparado, espera slot SQL | Corregir día Córdoba en productor legacy M97 preservando gate MFA M101, guards y atomicidad. Prueba fuerza desfase de fecha, rechazo AAL1 sin efecto, AAL2 y traspaso a M113. No modificar fechas existentes ni activar políticas. Revisión y RED/GREEN/fullSQL antes de publicación |
...
| D02 | Alta / nueva dirección visual confirmada | D / B + titular | pendiente tras M124; D01 es base funcional | Página pública con base visual Folio e identidad profesional/clínica protagonista. Reforma coherente con B02b y su vista previa, jerarquía/hero/servicios/contacto/reserva/OG, móvil y escritorio. Conservar privacidad, consentimiento y datos reales; sin testimonios ni verificaciones inventadas. D01 no equivale a aprobación visual final del usuario |
...
Cloudflare guardado, verificado y titular confirma desaparición del error. PR166 reúne B01, B02a y D01 revisados; cerrar revisión del lock Linux y desfase de disponibilidad M125, verificar checks del SHA final y preparar autorización concreta de publicación. El futuro B03/M124 va por separado y deberá desactivar su auto-Preview Vercel antes de cualquier push. Motor Docker operativo; C01 aún requiere ensayo completo, sin sustituirlo por pruebas parciales ni repetir el smoke clínico cerrado.
