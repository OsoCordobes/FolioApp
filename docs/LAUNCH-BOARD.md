...
| B01 | Bloqueante resuelto / configuración comprobada + recuperación UI | B / D | Cloudflare corregido y titular confirma que desapareció el error; UI `82591bf` integrada como `feb8d60`; CI f769944 PASS | Typecheck/lint/build PASS; unidades 2234/2234; navegador 5/5 + registro alternativo 1/1; D aprueba SHA exacto. Widget permite foliosalud.com y dominio anterior, conserva Managed/sin pre-clearance/sitekey. Bloqueo del widget resuelto en prueba humana; publicación UI y recorrido completo de cuenta/onboarding siguen pendientes |
| B02 | Alta / defecto corregido local + evidencia pendiente | B / D | B02a `0bd5ca7` aprobado por D e integrado como `ddcf2dc`; CI f769944 PASS | Recuperar contraseña respeta límite, respuesta incierta y doble envío. Tipos/lint/build PASS, 2236 unidades, 2 focales y 7 navegador combinados. Registro/confirmación/onboarding real/MFA y entrega de correo siguen pendientes |
| B02b | Alta / reforma solicitada por titular | B / D + A | `6e6d4ec` aprobado por D e integrado local; CI conjunta pendiente | Primera elección antes de registro, tres recorridos por modalidad/condición asistencial, salida al inicio y precios canónicos. Bootstrap M126 con DB autoritativa, recuperación explícita de borrador, sin sobreescritura por horarios tardíos ni recreación del borrador al finalizar. Tipo/lint/build y 2249 unidades PASS; navegador sintético según registro. Vista A aprobada; alta/correo reales y publicación pendientes |
| C01 | Bloqueante / evidencia pendiente | C / D | preparador y lock aprobados; Docker vuelve a estar inaccesible; ensayo pendiente | Preparador `6f2dc4e` y lock `c4eadd6` aprobados e integrados; CI Linux PASS. Recuperación anterior de Docker conservada como antecedente; último preflight falla por pipe LinuxEngine inexistente y RAM libre baja. Sin descarga ni nuevo arranque. Propuesta PG17/Auth/Storage dedicada preparada; restauración completa/Auth/Storage/login/descifrado todavía no acreditados |
...
| B07 | Alta / defecto comprobado y corregido local | B / D | M125 `25f0ed5` aprobado, integrado como `fd7038b`; CI f769944 PASS | RED exacto previo, GREEN y118/118 migraciones+66/66 specs PASS sin SKIP, incluye M113. Gate MFA M101/guards/atomicidad conservados, AAL1/AAL2 y rollback comprobados. Sólo día Córdoba nuevo; sin modificar datos/flags existentes. Autorización y aplicación de migración productiva pendientes |
