...
- Evento de llamado inmutable con autor y UUID de operación. La misma operación con igual intención devuelve el mismo código, destino y cursor sin nuevo sonido; reutilizar el UUID con intención distinta es conflicto. Para llamar nuevamente se crea otra operación de forma consciente. Se bloquean organización→actor→profesional→turno→paciente→identidad en lectura compartida y se serializa la operación/cursor privados; no se cambia EN_SALA. Deadlock o timeout exige leer la misma operación antes de decidir, nunca un reintento con UUID nuevo.
