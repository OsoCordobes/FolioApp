...
Estado actualizado al 12 de septiembre de 2026: **hay una copia cifrada nueva de producción verificada y una tarea Windows v3 con aviso de antigüedad; la restauración completa de Supabase sigue pendiente**. La tarea funciona sin Codex. La captura terminó a las 16:20 UTC y la tarea v3 reconoció la copia a las 17:00 UTC, sin avisar porque estaba reciente. Véase [programación y evidencia actual](RESPALDOS-ACTUALIZACION-2026-09-12.md). Los apartados siguientes distinguen el diseño y los ensayos sintéticos de la captura real; no deben interpretarse como una certificación de recuperación integral.
...
Una exclusión mantenida por el sistema operativo impide dos capturas simultáneas en esta PC. En Windows usa una tubería local con nombre derivado del destino; al terminar abruptamente el proceso, Windows la libera. La siguiente ejecución puede crear una copia nueva y conserva los archivos incompletos anteriores. No se desaloja una captura por antigüedad: una copia lenta sigue protegida. `.backup-owner.json` es sólo un diagnóstico, no bloquea ejecuciones tras un apagado. Un lock de directorio de la versión anterior sólo se archiva automáticamente si su PID ya no existe; ante datos incompletos o un PID activo se requiere revisión. En Linux se usa un socket Unix abstracto con el hash completo del destino; en otros Unix se conserva el puerto de loopback exclusivo. Un fallo al adquirir cualquiera de ellos rechaza la captura. Antes de actualizar operadores Linux, detener todos los procesos que usen el lock anterior por TCP: las versiones TCP y socket abstracto no se excluyen mutuamente sobre el mismo destino. Los operadores de un destino deben compartir el mismo namespace de red; si varios contenedores con namespaces distintos comparten un volumen, designar un único operador. Este mecanismo protege un destino local en una PC, no una carpeta compartida entre varios equipos.
...
`status` valida la copia indexada; `catch-up` ejecuta una captura cuando corresponde. La tarea Windows registrada comprueba cada hora y al iniciar sesión, intenta una copia desde las 20 horas y solicita un aviso si la copia verificada supera 24 horas o no existe. La página no depende de que el equipo esté encendido; la frescura del respaldo y el aviso local sí. Todavía falta medir el RPO sostenido y ensayar la restauración completa.
