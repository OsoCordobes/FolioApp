...
- Añadir revisión monotónica de DOB a `paciente_identidad` (`dob_revision`) y revisión monotónica del vínculo `paciente.identidad_id` (`identity_link_revision`). Triggers de `BEFORE UPDATE` calculan las revisiones desde `OLD`, aunque un cliente intente escribirlas. A→B→A no revive una verificación vieja; tampoco desvincular y volver a vincular la misma identidad.
