...
Estado actual omite `operationId` y `pagoOrigen`. `origen` puede ser AGENDA, CLINICAL, LEGACY o HISTORICO; es NULL en un turno abierto sin marcador. `clasificacion` también puede ser NULL antes de cerrar. `pagoOrigen` distingue CREADO, EXISTENTE, SIN_CARGO y SIN_DECISION. En el estado actual, `pago` es NULL si no existe o si el rol vigente es COORDINADOR, y `puedeRegistrar` indica la capacidad vigente del rol, no que deba registrar un nuevo pago sobre un asiento ya resuelto.

El recibo es inmutable: conserva `puedeRegistrar` y la proyección de pago del momento original. Su consulta vuelve a autorizar el acceso, pero una promoción o degradación de rol no reescribe esos campos históricos. Para habilitar controles, el caller usa las capacidades actuales y una lectura autorizada del estado; nunca concede permisos a partir del recibo.
...
4. El guard exige autoridad privada de M120 o WRITE real de M106 para entrar o volver a escribir CERRADO. Un GUC del cliente no concede autoridad. INSERT directo de pago queda bloqueado aun con ON CONFLICT DO NOTHING; mover `pago.turno_id` tampoco permite evadirlo. M120 por sí sola conserva el UPDATE del asiento existente; el candidato agrega [M121](M121-PAYMENT-SETTLEMENT.md), cuyo guard posterior exige la operación autorizada de saldo y bloquea ese UPDATE directo.
...
M120 fue aprobada independientemente en el commit `808f298`: replay completo de **114 migraciones / 62 specs SQL** y **8/8 casos de concurrencia**, todos aprobados. SHA-256 de la migración: `3a8047fee2323c671def88c6976f91a2dcb9b8d401cd70ba51aa7038b7260464`. El informe de ejecución está en `.flow/launch-reliability/atomic-close-report.md`. Las dos specs SQL usan `tests/fixtures/M120_turno_close.sql` dentro de una transacción reversible. `tests/integration/turno-close-concurrency.mjs` exige una base local nueva con prefijo `folio_test_launch_m120_`, conserva su fixture sintético y no resetea bases ni modifica roles compartidos.

Con M121 incorporada, el replay completo aprobó **115 migraciones / 63 specs**. Además se verificó el orden de actualización desde `28ab28a` (92 migraciones, M118 incluida) hacia `34dc605` (23 faltantes): **115 versiones exactas y 63 specs aprobadas**, conservando la validación predeterminada de cuerpos de funciones. Es evidencia en una base nueva local; no convierte ni inventaría datos de producción. Detalle: `.flow/launch-reliability/published-upgrade-report.md`.
