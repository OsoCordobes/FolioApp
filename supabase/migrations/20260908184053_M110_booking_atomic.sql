...
-- The migration runner owns the transaction, including its migration ledger.
...
CREATE FUNCTION folio_booking_private.promote_pedido_atomic(p_org uuid,p_pedido uuid,p_profesional uuid,p_servicio uuid,p_inicio timestamptz,p_expected_patient uuid,p_identity jsonb)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$
DECLARE pe public.pedido;sv public.servicio;org public.organization;prof public.member;patient public.paciente;actor public.member;visit public.turno;
 receipt folio_booking_private.conversion;identity_id uuid;patient_id uuid;turn_id uuid;duration int;price int;
...
BEGIN
 ...
 IF NOT service_call THEN
  ...
  IF auth.uid() IS NULL THEN RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Current staff access required';END IF;
  ...
  IF NOT FOUND OR actor.role NOT IN('OWNER','DIRECTOR','PROFESIONAL','COORDINADOR','ASISTENTE') THEN
   RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Current accepted staff required';END IF;
 END IF;
 ...
 IF service_call AND (pe.canal<>'WEB' OR org.opt_out_public_listing OR NOT EXISTS(SELECT 1 FROM folio_booking_private.submission r WHERE r.pedido_id=pe.id AND r.organization_id=p_org)) THEN
  RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Public confirmation authority missing';END IF;
 SELECT * INTO receipt FROM folio_booking_private.conversion WHERE pedido_id=pe.id AND organization_id=p_org;
 IF FOUND THEN
  -- Recover the committed operation only after checking CURRENT resources.
  -- Submitted IDs never define the agenda authority of an existing receipt.
  SELECT * INTO visit FROM public.turno WHERE id=receipt.turno_id AND organization_id=p_org
   AND paciente_id=receipt.paciente_id AND deleted_at IS NULL FOR SHARE;
  IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Current visit required';END IF;
  SELECT * INTO prof FROM public.member WHERE id=visit.profesional_id AND organization_id=p_org AND deleted_at IS NULL
   AND es_colegiado AND (accepted_at IS NOT NULL OR invited_by_id IS NULL) FOR SHARE;
  IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Current receipt professional required';END IF;
  IF NOT service_call AND NOT (actor.role IN('OWNER','DIRECTOR') OR actor.id=prof.id
   OR (actor.role IN('ASISTENTE','COORDINADOR') AND public.user_has_scope_over(p_org,prof.id))) THEN
   RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Current receipt agenda scope required';END IF;
  SELECT * INTO patient FROM public.paciente WHERE id=receipt.paciente_id AND organization_id=p_org
   AND deleted_at IS NULL AND pseudonimizado_en IS NULL FOR SHARE;
  IF NOT FOUND OR (patient.caja_fuerte_profesional IS NOT NULL AND (service_call OR patient.caja_fuerte_profesional IS DISTINCT FROM actor.id)) THEN
   RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Current receipt patient scope required';END IF;
  IF NOT service_call AND actor.role='PROFESIONAL' AND patient.profesional_principal_id IS DISTINCT FROM actor.id
   AND NOT public.profesional_attended_paciente(patient.id,p_org) THEN
   RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Current patient assignment required';END IF;
  ...
  IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Current receipt identity required';END IF;
  -- Service retries belong only to the original automatic public conversion,
  -- never a pending public request subsequently resolved by a staff member.
  IF service_call AND NOT EXISTS(SELECT 1 FROM folio_booking_private.submission r WHERE r.pedido_id=pe.id
   AND r.organization_id=p_org AND r.result->>'autoConfirmado'='true') THEN
   RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Original public conversion authority required';END IF;
  ...
  IF ROW(p_profesional,p_servicio,p_inicio) IS DISTINCT FROM ROW(pe.profesional_id,pe.servicio_id,pe.fecha_propuesta) THEN
   RAISE EXCEPTION USING ERRCODE='40001',MESSAGE='Booking was already converted with different details';END IF;
 ...
 IF service_call AND (NOT org.auto_confirmar_reservas OR pe.paciente_id IS NOT NULL OR p_profesional IS DISTINCT FROM pe.profesional_id OR p_servicio IS DISTINCT FROM pe.servicio_id OR p_inicio IS DISTINCT FROM pe.fecha_propuesta) THEN
 ...
 IF NOT service_call THEN
  IF NOT (actor.role IN('OWNER','DIRECTOR') OR actor.id=p_profesional
   OR (actor.role IN('ASISTENTE','COORDINADOR') AND public.user_has_scope_over(p_org,p_profesional))) THEN
   RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Current destination agenda scope required';END IF;
  IF pe.profesional_id IS NOT NULL AND NOT (actor.role IN('OWNER','DIRECTOR') OR actor.id=pe.profesional_id
   OR (actor.role IN('ASISTENTE','COORDINADOR') AND public.user_has_scope_over(p_org,pe.profesional_id))) THEN
   RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Current source request scope required';END IF;
 END IF;
...
END $$;
REVOKE ALL ON FUNCTION folio_booking_private.promote_pedido_atomic(uuid,uuid,uuid,uuid,timestamptz,uuid,jsonb) FROM PUBLIC,anon,authenticated,service_role;
GRANT USAGE ON SCHEMA folio_booking_private TO authenticated,service_role;
GRANT EXECUTE ON FUNCTION folio_booking_private.promote_pedido_atomic(uuid,uuid,uuid,uuid,timestamptz,uuid,jsonb) TO authenticated,service_role;
...
RETURNS jsonb LANGUAGE sql SECURITY INVOKER SET search_path=pg_catalog AS $$
 SELECT folio_booking_private.promote_pedido_atomic(p_org,p_pedido,p_profesional,p_servicio,p_inicio,p_expected_patient,p_identity)
$$;
...
CREATE FUNCTION folio_booking_private.public_booking_receipt(p_slug text,p_operation uuid,p_hash text) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$
DECLARE r folio_booking_private.submission;BEGIN
 IF current_setting('role',true) IS DISTINCT FROM 'service_role' THEN
  RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='Public booking gateway required';END IF;
 IF p_operation IS NULL OR coalesce(p_hash,'')!~'^[a-f0-9]{64}$' THEN
  RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='Stable booking receipt required';END IF;
 ...
 JOIN public.pedido pe ON pe.id=r0.pedido_id AND pe.organization_id=o.id AND pe.canal='WEB'
 WHERE o.slug=p_slug AND r0.operation_id=p_operation FOR SHARE OF o,r0,pe;
 ...
 -- This is proof that the original public request was received, not clinical
 -- access or the current appointment status. Later staff/vault changes cannot
 -- grant this visitor patient/visit IDs, nor trigger another conversion.
 RETURN jsonb_build_object('id',r.pedido_id,'autoConfirmado',(r.result->>'autoConfirmado')::boolean);
END $$;
REVOKE ALL ON FUNCTION folio_booking_private.public_booking_receipt(text,uuid,text) FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION folio_booking_private.public_booking_receipt(text,uuid,text) TO service_role;
CREATE FUNCTION public.public_booking_receipt(p_slug text,p_operation uuid,p_hash text) RETURNS jsonb
LANGUAGE sql SECURITY INVOKER SET search_path=pg_catalog AS $$ SELECT folio_booking_private.public_booking_receipt(p_slug,p_operation,p_hash) $$;
...
CREATE FUNCTION folio_booking_private.submit_public_booking(p_slug text,p_operation uuid,p_hash text,p_profesional uuid,p_servicio uuid,p_inicio timestamptz,p_data jsonb,p_identity jsonb) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$
DECLARE org public.organization;sv public.servicio;existing jsonb;pe_id uuid;promoted jsonb;outcome jsonb;local_start timestamp;BEGIN
 ...
 RETURN outcome;
END $$;
REVOKE ALL ON FUNCTION folio_booking_private.submit_public_booking(text,uuid,text,uuid,uuid,timestamptz,jsonb,jsonb) FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION folio_booking_private.submit_public_booking(text,uuid,text,uuid,uuid,timestamptz,jsonb,jsonb) TO service_role;
...
LANGUAGE sql SECURITY INVOKER SET search_path=pg_catalog AS $$ SELECT folio_booking_private.submit_public_booking(p_slug,p_operation,p_hash,p_profesional,p_servicio,p_inicio,p_data,p_identity) $$;
...
CREATE FUNCTION folio_booking_private.booking_claim_followups(p_limit int DEFAULT 10) RETURNS SETOF public.booking_followup_job
LANGUAGE sql SECURITY DEFINER SET search_path=pg_catalog AS $$
 ...
 WHERE current_setting('role',true)='service_role' AND j.status IN('pending','leased') AND j.available_at<=now() AND (j.lease_until IS NULL OR j.lease_until<=now()) ORDER BY j.available_at,j.id LIMIT greatest(1,least(p_limit,10)) FOR UPDATE OF j SKIP LOCKED)
...
$$;
CREATE FUNCTION folio_booking_private.booking_finish_followup(p_id uuid,p_lease uuid,p_success boolean,p_terminal boolean DEFAULT false) RETURNS boolean
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$ BEGIN
 ...
 WHERE current_setting('role',true)='service_role' AND id=p_id AND status='leased' AND lease_token=p_lease AND lease_until>now();RETURN FOUND;
END $$;
REVOKE ALL ON FUNCTION folio_booking_private.booking_claim_followups(int),folio_booking_private.booking_finish_followup(uuid,uuid,boolean,boolean) FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION folio_booking_private.booking_claim_followups(int),folio_booking_private.booking_finish_followup(uuid,uuid,boolean,boolean) TO service_role;
CREATE FUNCTION public.booking_claim_followups(p_limit int DEFAULT 10) RETURNS SETOF public.booking_followup_job
LANGUAGE sql SECURITY INVOKER SET search_path=pg_catalog AS $$ SELECT * FROM folio_booking_private.booking_claim_followups(p_limit) $$;
...
LANGUAGE sql SECURITY INVOKER SET search_path=pg_catalog AS $$ SELECT folio_booking_private.booking_finish_followup(p_id,p_lease,p_success,p_terminal) $$;
