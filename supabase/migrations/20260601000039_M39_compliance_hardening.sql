-- ════════════════════════════════════════════════════════════════════════════
-- Folio · M39 · Compliance hardening (Ley 25.326 + Ley 26.529)
-- ════════════════════════════════════════════════════════════════════════════
-- Reconciliación de la auditoría legal del 2026-05-21 (PR #2, branch
-- claude/folio-legal-compliance-audit-4JV32). El draft original numeraba esta
-- migration como `20260521000022_M22_compliance_hardening`, lo que colisiona
-- con la M22 que ya está en master (`M22_rls_hardening`) y quedó 62 commits
-- atrás. Se renumera a M39 (2026-06-01) y se verifica pieza por pieza contra
-- el schema vigente antes de incluir cada cambio (ver cuerpo del PR para la
-- matriz de verificación). NO cambia el modelo de datos; cierra gaps:
--
...
--
--   2. Hardening de `sesion_update_clinical`: agrega explícitamente
--      `locked_at IS NULL` al USING, PRESERVANDO la lógica vigente (M10). El
--      trigger BEFORE UPDATE ya bloquea modificaciones a una sesión lockeada
--      (M10) y el unlock (M22 prevent_sesion_unlock), pero defensa-en-
--      profundidad a nivel policy es trivial y elimina dependencia del orden
--      de ejecución. La policy se re-crea idéntica a M10 + el guard nuevo.
--
--   3. Columnas de consentimiento al booking público: `pedido.consent_aceptado_en`,
--      `consent_ip`, `consent_user_agent`, `consent_version`. La acción pública
--      del booking (CRIT-1) ahora obliga al visitante a aceptar Privacidad/
--      Términos antes del INSERT (Ley 25.326 art. 5: consentimiento libre,
--      expreso, informado). ACOPLAMIENTO CRÍTICO: el CHECK rechaza todo pedido
--      WEB sin consent — esta migration viaja junto con el wire-up de la
--      checkbox en components/booking/booking-wizard.tsx +
--      app/(public)/book/[slug]/actions.ts (ver PR). El constraint es NOT VALID
--      para no romper filas WEB históricas previas al deploy.
--
...
--      versión existente (M13, `pseudonimizar_paciente`) solo borraba PII de
--      pacientes. Cuando el profesional ejerce su propio derecho ARCO de
--      supresión, hay que borrar el `profile.*_cifrado` PII manteniendo la
--      PHI clínica huérfana 10 años.
-- ════════════════════════════════════════════════════════════════════════════
...
DROP TRIGGER IF EXISTS organization_audit ON organization;
...
DROP TRIGGER IF EXISTS profile_audit ON profile;
CREATE TRIGGER profile_audit
  AFTER UPDATE OR DELETE ON profile
  FOR EACH ROW EXECUTE FUNCTION audit_log_trigger_profile();

-- ─── 2. Hardening sesion_update_clinical (PRESERVA lógica M10 + locked_at) ─
-- Se re-crea la policy IDÉNTICA a la versión vigente (M10) y se le agrega el
-- único guard nuevo: `AND locked_at IS NULL` en el USING. NO se modifica el
-- resto de la lógica (org scope + can_read_clinical + OWNER-o-profesional-del-
-- turno). Verificado contra master: ninguna migration entre M10 y M38 redefine
-- esta policy.
...
CREATE POLICY sesion_update_clinical
  ...
  USING (
    ...
    AND locked_at IS NULL                         -- defense-in-depth (M39)
    AND (
...
-- vía tabla `consentimiento`). NOT VALID: filas WEB viejas no se re-validan.
-- Guard idempotente para que el replay del CI no falle por constraint dup.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'pedido_web_requires_consent'
