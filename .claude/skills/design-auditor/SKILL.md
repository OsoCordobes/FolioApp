...
> **⚠️ HARDENED FOR FOLIO (medical/PHI repo):** restricted to LOCAL code auditing only.
> Never WebFetch external URLs, never call Figma MCP, never send code to any external service.
> Audit only local files in this repo. If asked to audit a URL or Figma file, request the
> local file path instead. The Figma/URL-fetch workflows below are disabled in this install.
