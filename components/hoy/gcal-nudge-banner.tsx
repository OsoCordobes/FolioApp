...
export function GcalNudgeBanner({ modo, memberId }: GcalNudgeBannerProps) {
  ...
  return (
    ...
    >
      <span aria-hidden className="fi-gcal-nudge-icon">
      ...
      <div className="fi-gcal-nudge-copy">
        {copy.texto}
        {actionError ? (
          <span role="alert" className="fi-gcal-nudge-error">
            {actionError}
          </span>
        ) : null}
      </div>
      <button
        type="button"
        className="fi-btn fi-btn-primary fi-gcal-nudge-cta"
        onClick={onConnect}
        disabled={pending}
      >
      ...
      <button
        type="button"
        onClick={onDismiss}
        className="fi-gcal-nudge-dismiss"
