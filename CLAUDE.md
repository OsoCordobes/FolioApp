...
- ⚠️ **Back up `.env.local` before running any CLI that writes into the project dir** (`vercel integration add`, `vercel env pull`, `supabase link/init`, scaffolders). On 2026-08-13 `vercel integration add upstash/upstash-kv` silently overwrote it with the Development env and destroyed the only readable copy of `FOLIO_ENC_KEY`/`FOLIO_ENC_HMAC_KEY` — they are `sensitive` in Vercel (write-only) and unrecoverable. `.env.local` is gitignored by design (`.gitignore:34`), so **git cannot undo this**. One `cp .env.local .env.local.bak` first makes it a non-event. See `docs/ROTACION-CLAVES.md`.
