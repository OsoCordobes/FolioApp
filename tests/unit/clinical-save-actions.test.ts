...
function fixture(){
 ...
 let throws=false;let followupFails=false;let cacheFails=false;
 ...
 runInNewContext(source,{exports,require:(name:string)=>{
  ...
  if(name==="next/cache")return {revalidatePath:()=>{if(cacheFails)throw Error("cache unavailable");}};
  if(name==="@/lib/db/turno-close")return {getTurnoCloseStatus:async()=>{if(followupFails)throw Error("status unavailable");return {ok:true,data:{estado:"CERRADO",clasificacion:"REQUIERE_REGISTRO"}};}};
 ...
 return {input,writes,followups,save:exports.saveSesionFichaAction,close:exports.saveSesionYCerrarAction,setResult:(value:unknown)=>{result=value;},failTransport:()=>{throws=true;},failFollowup:()=>{followupFails=true;},failCache:()=>{cacheFails=true;}};
}
...
test("committed atomic close remains successful when later payment followup fails",async()=>{
 const f=fixture();f.setResult({ok:true,data:{id:randomUUID(),revision:1,updatedAt:"2026-09-08T18:00:00Z",operationId:f.input.operacionId,closed:true}});f.failFollowup();const result=await f.close(f.input);assert.equal(result.ok,true);assert.equal(result.data?.cerrado,true);assert.match(result.data?.aviso??"",/guardada y cerrada/);assert.equal(f.followups.length,0);
});
for(const action of ["save","close"] as const)for(const autosave of [false,true])test(`${action} committed receipt survives cache failure (autosave=${autosave})`,async()=>{
 const f=fixture();f.setResult({ok:true,data:{id:randomUUID(),revision:1,updatedAt:"2026-09-08T18:00:00Z",operationId:f.input.operacionId,closed:action==="close"}});f.failCache();const result=await f[action]({...f.input,autosave});assert.equal(result.ok,true);assert.equal(f.writes.length,1);assert.equal(f.followups.length,0);
});
