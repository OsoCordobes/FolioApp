...
function fixture(fail=false, revoked=false, initialRole="PROFESIONAL", colegiado=true, revokeSession=false, revokeColegiado=false) {
 let reads=0,contexts=0;let rendered:FichaPdfData|undefined;
 ...
 '@/lib/db/audit':{writeAuditEntry:async()=>{}},'@/lib/supabase/server':{createSupabaseServerClient:async()=>client},
 ...
 return{run:(query='')=>exports.GET!(new Request(`http://127.0.0.1/api/pacientes/${patient}/ficha-pdf${query}`),{params:Promise.resolve({id:patient})}),rendered:()=>rendered,reads:()=>reads};
}
