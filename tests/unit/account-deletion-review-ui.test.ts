...
test('missing or failed profile read never renders an empty successful request state',async()=>{
 for(const failure of [false,true]) {
  ...
  const imports:Record<string,unknown>={'react/jsx-runtime':{jsx,jsxs:jsx},'next/navigation':{redirect:()=>{throw Error('unexpected redirect');}},'./datos-client':{DatosClient:'DatosClient'},'@/lib/supabase/server':{createSupabaseServerClient:async()=>({auth:{getUser:async()=>({data:{user:{id:'actor'}}})}}),createSupabaseServiceClient:()=>({from:()=>query})}};
  runInNewContext(compiled('app/(app)/configuracion/datos/page.tsx'),{exports,require:(name:string)=>{if(name in imports)return imports[name];throw Error(name);}});
  const rendered=JSON.stringify(await exports.default!());
...
for(const missing of [false,true,'error','foreign'])test(`request and withdrawal only update the verified user's marker (result ${missing})`,async()=>{
 ...
 const imports:Record<string,unknown>={'next/cache':{revalidatePath:()=>{}},'@/lib/auth/mfa-access':{verifyMfaSession:async()=>({ok:true,data:{user:{id:'actor'}}})},'@/lib/me/personal-export':{},'@/lib/crypto':{},'@/lib/support':{},'@/lib/supabase/server':{createSupabaseServerClient:async()=>({auth:{getUser:async()=>({data:{user:{id:'actor'}},error:null})}}),createSupabaseServiceClient:()=>({from:(table:string)=>{calls.push(table);return q;}})}};
