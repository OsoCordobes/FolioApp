...
Runtime changes cover Dashboard, its appointment list/row, and the pure
predecessor helper. The harness declares esbuild 0.28.0 as a direct development
dependency. No migration, runtime dependency upgrade, environment change, or new
synchronization hook is required.

## Reproduce

Install the committed lockfile with `pnpm install --frozen-lockfile`. The browser
...
a predecessor snapshot. Final focused result: **30 passed**.
...
Its eight scenarios run in both development and production React bundles:
...
Observed results on this branch: **16 browser scenarios**, **1,577 complete unit
...
No migration, recovery script, or environment file belongs in this selective port. An independent reviewer reproduced payment regressions on
...
## PR review: dependent navigation

The browser reproduced one navigation with no attention request when opening
the chart while arrival was pending. Rows now show a disabled Guardando control,
with disabled menu/keyboard activation, while other rows remain available.
A rejected successor returns false synchronously; the row navigates only when
the transition was accepted for dispatch. This is not a server persistence ACK.
The browser verifies the real pending click, keyboard guard, and next action
after acknowledgement in both React modes.

Port the pending prop and accepted-transition contract in turno-list/turno-row,
Dashboard pending IDs and navigation guards, and the new focal/browser tests.
The harness uses require('esbuild'), never an internal pnpm path or NODE_PATH.
Lint invokes the package manager executable. Add esbuild 0.28.0 as a direct
dev dependency with the target branch package manager; preserve its lockfile
versions and central isolation wrapper instead of copying the hotfix lockfile.
