...
test("switching login and signup remounts a fresh widget", async ({ page }) => {
  await page.route(scriptUrl, (route) => route.fulfill({ status: 200, contentType: "application/javascript", body: syntheticApi }));
  ...
  await page.getByRole("button", { name: /crear cuenta/i }).first().click();
  await expect.poll(() => page.evaluate(() => (window as unknown as Window & { __turnstileStats: { renders: number } }).__turnstileStats.renders)).toBe(2);
  expect(await page.evaluate(() => (window as unknown as Window & { __turnstileStats: { removes: number } }).__turnstileStats.removes)).toBe(1);
});
...
test("the alternate signup keeps its fields and renews the challenge after a failed response", async ({ page, baseURL }) => {
  const appOrigin = new URL(baseURL!).origin;
  await page.route(scriptUrl, (route) => route.fulfill({ status: 200, contentType: "application/javascript", body: syntheticApi }));
  let posts = 0;
  await page.route((url) => url.origin === appOrigin && url.pathname === "/login", async (route) => {
    if (route.request().method() === "POST") {
      posts++;
      await new Promise((resolve) => setTimeout(resolve, 500));
      await route.fulfill({ status: 503, body: "synthetic failed response" });
  ...
  await page.goto("/login");
  await page.getByRole("button", { name: /crear cuenta/i }).first().click();
  await page.locator('input[type="email"]').fill("alternate@example.test");
  await page.locator('input[type="password"]').fill("synthetic-password-only");
  await page.locator('input[type="checkbox"]').first().check();
  await expect.poll(() => page.evaluate(() => (window as unknown as Window & { __turnstileStats?: { renders: number } }).__turnstileStats?.renders)).toBe(1);
  await page.getByRole("button", { name: "Crear cuenta", exact: true }).evaluate((button: HTMLButtonElement) => {
    button.click();
    button.click();
  });
  await expect.poll(() => posts).toBe(1);
  await expect(page.getByText(/No pudimos confirmar si la cuenta se creó/)).toBeVisible();
  await expect.poll(() => page.evaluate(() => (window as unknown as Window & { __turnstileStats: { renders: number } }).__turnstileStats.renders)).toBe(2);
  await expect(page.locator('input[type="email"]')).toHaveValue("alternate@example.test");
  await expect(page.locator('input[type="password"]')).toHaveValue("synthetic-password-only");
});
