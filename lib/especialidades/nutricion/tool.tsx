 ...
 *      con el <SerieEvolucion> genérico de la biblioteca (C3).
...
const CHIP_ESTADO_OBJETIVO: Record<EstadoObjetivoNutri, CSSProperties> = {
...
export function NutricionTool({
  ...
  const agregarObjetivo = () => {
    ...
    const objetivo: ObjetivoNutri = { texto: textoNuevoObjetivo.slice(0, 500), estado: "en_curso" };
    emit({ ...draft, objetivos: [...objetivos, objetivo] });
    setNuevoObjetivo("");
  };

  const setEstadoObjetivo = (i: number, estado: EstadoObjetivoNutri) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24, minWidth: 0 }}>
      ...
      <section className="pc-card">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <SerieEvolucion
            ...
            ariaLabel={`Evolución de peso, IMC y cintura en ${series.length} ${series.length === 1 ? "sesión" : "sesiones"}`}
      ...
      <section className="pc-card">
        <header className="pc-card-head">
          <span className="fi-eyebrow">Objetivos nutricionales</span>
        </header>
            ...
            {objetivos.map((o, i) => (
              ...
              >
                ...
                <select
                  ...
                  value={o.estado}
                  onChange={(e) => setEstadoObjetivo(i, e.target.value as EstadoObjetivoNutri)}
                  disabled={readOnly}
                  aria-label={`Estado del objetivo: ${o.texto}`}
                >
                  {ESTADOS_OBJETIVO_NUTRI.map((estado) => (
                    <option key={estado} value={estado}>
                      {ESTADO_OBJETIVO_NUTRI_LABELS[estado]}
                    </option>
                  ))}
                </select>
                {!readOnly ? (
              ...
              {ultimosObjetivos.objetivos.map((o, i) => (
                ...
                >
                  ...
                  <span className="fi-pill" style={CHIP_ESTADO_OBJETIVO[o.estado]}>
                    {ESTADO_OBJETIVO_NUTRI_LABELS[o.estado]}
                  </span>
                </div>
              ))}
