import { progressPackageOperation } from "@/lib/patient/export-package-delivery";
...
export async function POST(request: Request, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    const body = await packageBody(request,
      ["patientId", "operationId", "leaseToken", "revision", "sourceOrdinal"]);
    ...
    const progress = await progressPackageOperation(context.data.client, context.data.session,
      ...
      body.leaseToken, body.revision as number, body.sourceOrdinal as number);
    return packageResult(progress, data => packageJson({ ok: true, progress: data }));
